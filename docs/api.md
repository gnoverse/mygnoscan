...
| endpoint | description |
|---|---|
| `GET /api/graph/transfers?network=&window=&topN=&min_value=&ego=&addresses=` | top-N addresses by transfer volume in the window (or the 1-hop neighborhood of `ego`, which ignores `topN`). Returns `{nodes: [{id, volume}], edges: [{from, to, value, tx_count}]}`. `topN` defaults to 100, capped at 1000. There is no `hops` parameter — the `ego` neighborhood is hardcoded to 1 hop. `addresses` is a comma-separated address list (capped at 20): when present, returns the induced subgraph over exactly that address list instead of top-N/ego mode, and takes precedence over `topN`/`ego` if all three are somehow present |
| `GET /api/graph/callers?network=&window=&topN=&min_calls=&entities=` | top-N callers by call volume and the realms they called. Returns `{nodes: [{id, type, calls}], edges: [{caller, pkg_path, calls}]}` where `type` is `"caller"` or `"realm"`. `topN` defaults to 200, capped at 1000. No `ego` support yet. `entities` is a comma-separated list of caller addresses and/or realm paths (capped at 20): when present, returns the induced subgraph over exactly that entity list instead of top-N mode. Because `caller_edges` is bipartite, an all-caller or all-realm selection returns zero edges by design |
| `GET /api/graph/transfers/ranking?network=&window=&topN=&search=` | ranks addresses by total volume (sent+received) in the window, split into `sent`/`received`/`volume`. Returns `[{address, sent, received, volume}]`. `topN` defaults to 100, capped at 1000; ignored when `search` is set. `search` does a bounded (50-result) substring match against addresses, including ones outside the top ranking |
| `GET /api/graph/callers/ranking?network=&window=&topN=&search=` | ranks callers and realms together by call volume, independently — a realm's rank reflects its own total, not just calls from top-ranked callers. Returns `[{id, type, calls}]` where `type` is `"caller"` or `"realm"`. `topN` defaults to 200, capped at 1000; ignored when `search` is set. Same bounded `search` semantics as the transfers ranking |
