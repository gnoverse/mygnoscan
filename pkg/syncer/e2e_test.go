...
// faithfully.
func newE2ESyncer(t *testing.T) (*Syncer, *indexer.Fake, *store.DB) {
	t.Helper()
	...
	db.SetConfiguredNetworks([]config.NetworkConfig{{ID: "e2e"}})

	return NewSyncer(client, db, analyzer.NewAnalyzer(db), "e2e"), fake, db
}

// seedMixedChain writes one of every message type the syncer handles, each in
// its own block so block times are unambiguous.
func seedMixedChain(f *indexer.Fake) {
	...
	f.Add(
		indexer.Package(10, when(10), "g1deployer", "gno.land/r/demo/boards"),
		indexer.Call(11, when(11), "g1alice", "gno.land/r/demo/boards", "Post"),
		indexer.Send(12, when(12), "g1alice", "g1bob", "1000ugnot"),
		fakeMsgRun(13, when(13), "g1carol"),
	)
}

func fakeMsgRun(height int, when, caller string) indexer.Transaction {
	return indexer.Transaction{
		...
		Messages: []indexer.TxMessage{{
			...
			Value: indexer.MessageValue{Typename: "MsgRun", Caller: caller, Package: &indexer.MemPackage{
				Name: "main", Path: "gno.land/e/" + caller + "/run",
				Files: []indexer.MemFile{{Name: "main.gno", Body: "package main\n\nimport \"gno.land/p/demo/avl\"\n\nfunc main() {}\n"}},
			}},
		}},
	}
}
...
func TestSyncAllWritesEveryMessageType(t *testing.T) {
	...
	for _, table := range []string{"packages", "calls", "bank_sends", "msg_runs", "transactions"} {
		var n int
		if err := db.SQL().QueryRow(`SELECT COUNT(*) FROM ` + table + ` WHERE network = 'e2e'`).Scan(&n); err != nil {
...
func fakeMulticall(height int, when, caller, pkgPath, fn string, n int) indexer.Transaction {
	msgs := make([]indexer.TxMessage, n)
	for i := range msgs {
		msgs[i] = indexer.TxMessage{
			TypeURL: "exec", Route: "vm",
			Value: indexer.MessageValue{Typename: "MsgCall", Caller: caller, PkgPath: pkgPath, Func: fn},
		}
	}
	return indexer.Transaction{
...
func TestSyncCountsEveryMessageInAMulticall(t *testing.T) {
	syncer, fake, db := newE2ESyncer(t)
	fake.AddBlocks(indexer.Block{Hash: "genesis", Height: 1, ChainID: fake.ChainID, Time: "2026-08-01T00:00:00Z"})
	fake.AddBlocks(indexer.Block{Hash: "block-10", Height: 10, ChainID: fake.ChainID, Time: "2026-08-01T00:10:00Z", NumTxs: 1, TotalTxs: 10})
	...
	var count int
	if err := db.SQL().QueryRow(`SELECT COUNT(*) FROM calls WHERE network = 'e2e'`).Scan(&count); err != nil {
...
func TestSyncPicksUpNewBlocks(t *testing.T) {
	...
	fake.AddBlocks(indexer.Block{Hash: "block-20", Height: 20, ChainID: fake.ChainID, Time: when})
	fake.Add(indexer.Call(20, when, "g1dave", "gno.land/r/demo/boards", "Post"))
...
func TestSyncExtractsDependencies(t *testing.T) {
	...
	var edges int
	err := db.SQL().QueryRow(`SELECT COUNT(*) FROM dependencies WHERE network = 'e2e'
...
func countAll(t *testing.T, db *store.DB) map[string]int {
	t.Helper()
