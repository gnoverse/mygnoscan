...
func newTestSyncer(t *testing.T, network string) (*Syncer, *indexer.Fake, *store.DB) {
	t.Helper()
	...
	return NewSyncer(client, db, analyzer.NewAnalyzer(db), network), fake, db
}

func countNetworkRows(t *testing.T, db *store.DB, network string) int {
	t.Helper()

	total := 0
	for _, table := range store.NetworkScopedTables {
		var n int
		q := fmt.Sprintf(`SELECT COUNT(*) FROM %s WHERE network = ?`, table)
		if err := db.SQL().QueryRow(q, network).Scan(&n); err != nil {
...
func TestCheckChainResetDiscardsDataFromAPreviousChain(t *testing.T) {
	...
	store.SeedNetwork(t, db, "staging", 900)
	store.SeedNetwork(t, db, "topaz", 900) // another network, must survive

	if got := countNetworkRows(t, db, "staging"); got != len(store.NetworkScopedTables) {
		t.Fatalf("seeded rows = %d, want %d", got, len(store.NetworkScopedTables))
	}
	...
	if got := countNetworkRows(t, db, "staging"); got != len(store.NetworkScopedTables) {
		t.Fatalf("rows after no-op check = %d, want %d", got, len(store.NetworkScopedTables))
	}
	...
	if got := countNetworkRows(t, db, "topaz"); got != len(store.NetworkScopedTables) {
		t.Errorf("other network lost %d rows", len(store.NetworkScopedTables)-got)
	}
...
func TestCheckChainResetKeepsDataWhenIndexerIsUnreachable(t *testing.T) {
	...
	if got := countNetworkRows(t, db, "staging"); got != len(store.NetworkScopedTables) {
		t.Errorf("rows = %d after indexer failure, want %d", got, len(store.NetworkScopedTables))
	}
}
...
func TestCheckChainResetIgnoresALaggingIndexer(t *testing.T) {
	...
	store.SeedNetwork(t, db, "betanet", 3_000_000)
	...
	if got := countNetworkRows(t, db, "betanet"); got != len(store.NetworkScopedTables) {
		t.Errorf("rows = %d after lagging indexer, want %d", got, len(store.NetworkScopedTables))
	}

	syncer.warnOnHeightRegression(ctx) // logs only; must not panic or delete
	if got := countNetworkRows(t, db, "betanet"); got != len(store.NetworkScopedTables) {
		t.Errorf("rows = %d after regression warning, want %d", got, len(store.NetworkScopedTables))
	}
}
...
func TestDeleteNetworkDataIsScopedToOneNetwork(t *testing.T) {
	...
	store.SeedNetwork(t, db, "topaz", 100)
	store.SeedNetwork(t, db, "staging", 200)

	deleted, err := db.DeleteNetworkData("topaz")
	if err != nil {
		t.Fatalf("delete network data: %v", err)
	}
	if want := int64(len(store.NetworkScopedTables)); deleted != want {
	...
	if got := countNetworkRows(t, db, "staging"); got != len(store.NetworkScopedTables) {
		t.Errorf("staging rows = %d, want %d", got, len(store.NetworkScopedTables))
	}
}
...
func TestWalkTransactionsCoversEveryRowAcrossPages(t *testing.T) {
	...
	fake := &indexer.TruncatingServer{Tip: 8500, TxsPerBlock: 3}
	srv := httptest.NewServer(fake)
	t.Cleanup(srv.Close)
	client := indexer.NewClient(srv.URL)

	seen := make(map[string]int)
	pages := 0
	err := walkTransactions(context.Background(), nil, client.GetTransactionsFromHeight,
		func(txs []indexer.Transaction) {
			pages++
			for _, tx := range txs {
				seen[tx.Hash]++
			}
...
func TestWalkTransactionsStopsWhenCaughtUp(t *testing.T) {
	fake := &indexer.TruncatingServer{Tip: 100, TxsPerBlock: 1}
	srv := httptest.NewServer(fake)
	t.Cleanup(srv.Close)
	...
	err := walkTransactions(context.Background(), &cursor,
		indexer.NewClient(srv.URL).GetTransactionsFromHeight,
		func(txs []indexer.Transaction) { calls++ })
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if calls != 0 {
...
func TestValoperSubject(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			msg := indexer.TxMessage{Value: indexer.MessageValue{
				Typename: "MsgCall",
				Caller:   caller,
...
func newBlockTestSyncer(t *testing.T, network string, pageSize, pagesPerPass int) (*Syncer, *indexer.Fake, *store.DB) {
	t.Helper()
...
func TestSyncBlocksTerminatesAtGenesis(t *testing.T) {
	...
	done, err := db.GetSyncState(store.BlocksBackfillDoneKey("gnoland1"))
	if err != nil {
...
func TestSyncBlocksAbandonsPageOnProposerInternFailure(t *testing.T) {
	...
	s := NewSyncer(client, db, analyzer.NewAnalyzer(db), network)
...
func TestSyncBlocksHistoryCapBreaksPartwayThroughBackfill(t *testing.T) {
	...
	// indexer.BlockTime maps height to time. Aim the cutoff at the chain's midpoint,
	// then derive -block-history-days from the real wall clock so the cutoff
	// actually lands there.
	targetTime := indexer.BlockTime(tip / 2)
	...
	cutoffHeight := indexer.BlockHeightAt(cutoff)
	...
	for i := 0; i < 10; i++ {
		s.syncBlocks(context.Background())
		if done, _ := db.GetSyncState(store.BlocksBackfillDoneKey("gnoland1")); done == "1" {
			break
		}
	}
