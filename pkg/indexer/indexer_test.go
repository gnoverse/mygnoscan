...
func newCountingIndexer(t *testing.T) (*Client, *countingIndexer) {
	t.Helper()
	fake := &countingIndexer{}
	srv := httptest.NewServer(fake)
	t.Cleanup(srv.Close)
	return NewClient(srv.URL), fake
}
...
func TestGetRecentTransactionsPageWidensWindow(t *testing.T) {
	...
	txs, err := NewClient(srv.URL).GetRecentTransactionsPage(context.Background(), 3)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(txs) != 3 {
...
func TestClientBreakerShortCircuitsDeadIndexer(t *testing.T) {
	...
	if !errors.Is(mustErr(client.LatestBlockHeight(ctx)), ErrUnavailable) {
		t.Error("expected ErrUnavailable once the breaker is open")
	}
...
func TestTransactionsFromHeightAscendsFromTheCursor(t *testing.T) {
	...
	fake := &TruncatingServer{Tip: 200, TxsPerBlock: 1}
	srv := httptest.NewServer(fake)
	defer srv.Close()

	cursor := 50
	txs, truncated, err := NewClient(srv.URL).GetTransactionsFromHeight(context.Background(), &cursor)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if truncated {
...
func TestTransactionsFromHeightAtTip(t *testing.T) {
	...
	fake := &TruncatingServer{Tip: 500, TxsPerBlock: 1}
	srv := httptest.NewServer(fake)
	defer srv.Close()

	tip := fake.Tip
	txs, truncated, err := NewClient(srv.URL).GetTransactionsFromHeight(context.Background(), &tip)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
...
func TestTransactionsFromHeightRejectsAnOversizedBlock(t *testing.T) {
	...
	fake := &TruncatingServer{Tip: 1, TxsPerBlock: ElementCap + 10}
	srv := httptest.NewServer(fake)
	defer srv.Close()

	_, _, err := NewClient(srv.URL).GetTransactionsFromHeight(context.Background(), nil)
...
func TestSyncFetchersKeepTheirFilterAndOrder(t *testing.T) {
	...
	tests := []struct {
		name       string
		wantFilter string
		fetch      func(*Client, context.Context) ([]Transaction, bool, error)
	}{
		{"GetAllPackages", "MsgAddPackage", func(c *Client, ctx context.Context) ([]Transaction, bool, error) {
			h := 10
			return c.GetAllPackages(ctx, &h)
		}},
		{"GetMsgRunTransactions", "MsgRun", func(c *Client, ctx context.Context) ([]Transaction, bool, error) {
			h := 10
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fake := &TruncatingServer{Tip: 100, TxsPerBlock: 1}
			srv := httptest.NewServer(fake)
			defer srv.Close()

			if _, _, err := tt.fetch(NewClient(srv.URL), context.Background()); err != nil {
				t.Fatalf("unexpected error: %v", err)
			}
...
func TestRecentPageWidensUntilItFindsRows(t *testing.T) {
	...
	txs, err := NewClient(srv.URL).GetRecentTransactionsPage(context.Background(), 20)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if len(txs) < 20 {
...
func TestRecentPageServesACappedWindow(t *testing.T) {
	...
	fake := &TruncatingServer{Tip: 20000, TxsPerBlock: 200}
	srv := httptest.NewServer(fake)
	defer srv.Close()
	...
	if len(txs) != ElementCap {
		t.Fatalf("got %d rows, want the full capped page of %d", len(txs), ElementCap)
	}
...
func TestRecentPageStillFailsWhenCappedPageIsTooSmall(t *testing.T) {
	...
	_, err := NewClient(srv.URL).GetRecentTransactionsPage(context.Background(), ElementCap+1)
	if !errors.Is(err, ErrQueryTooLarge) {
		t.Fatalf("got %v, want ErrQueryTooLarge", err)
	}
}
...
func TestClientTimeoutsAreSeparate(t *testing.T) {
	...
	serve := NewClient("http://example.invalid/graphql")
	sync := NewSyncClient("http://example.invalid/graphql")
...
func TestRefusedRequestNamesTheStatus(t *testing.T) {
	...
	_, err := NewClient(srv.URL).LatestBlockHeight(context.Background())
	if err == nil {
...
func TestTransactionsFromHeightPageIsWholeBlocksOnly(t *testing.T) {
	...
	fake := &TruncatingServer{Tip: 20000, TxsPerBlock: 3}
	srv := httptest.NewServer(fake)
	defer srv.Close()

	txs, truncated, err := NewClient(srv.URL).GetTransactionsFromHeight(context.Background(), nil)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}
	if !truncated {
	...
	if len(txs) == 0 || len(txs) > ElementCap {
		t.Fatalf("got %d rows, want between 1 and the cap of %d", len(txs), ElementCap)
	}
