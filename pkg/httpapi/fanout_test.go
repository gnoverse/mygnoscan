...
func testNetworks(ids ...string) ([]config.NetworkConfig, map[string]*indexer.Client) {
	nets := make([]config.NetworkConfig, 0, len(ids))
	clients := make(map[string]*indexer.Client, len(ids))
	for _, id := range ids {
		nets = append(nets, config.NetworkConfig{ID: id, IndexerURL: "http://example.invalid/graphql/query"})
		clients[id] = indexer.NewClient("http://example.invalid/graphql/query")
	}
	return nets, clients
}
...
func TestFanOutRunsConcurrentlyAndKeepsOrder(t *testing.T) {
	...
	got := fanOut(context.Background(), nets, clients, newHealthTracker(),
		func(ctx context.Context, n config.NetworkConfig, c *indexer.Client) (string, error) {
			time.Sleep(delay)
			return n.ID, nil
		})
...
func TestFanOutSkipsNetworksWithoutAClient(t *testing.T) {
	...
	nets := []config.NetworkConfig{{ID: "configured"}, {ID: "clientless"}}
	clients := map[string]*indexer.Client{"configured": {}}
