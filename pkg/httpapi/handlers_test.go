...
func newTestAPI(t *testing.T) (*API, *store.DB) {
	...
	return NewAPI(db, map[string]*indexer.Client{}, nets, analyzer.NewAnalyzer(db)), db
}
...
func seedActivity(t *testing.T, db *store.DB, network string, calls, realms, sends int) {
	t.Helper()
...
func TestStorageBackedHandlers(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			check: func(t *testing.T, body []byte) {
				var regs []store.ValoperRegistration
...
func TestAccountsPagingAndSort(t *testing.T) {
	...
	get := func(t *testing.T, target string) []store.AccountInfo {
		t.Helper()
		rec := httptest.NewRecorder()
		mux.ServeHTTP(rec, httptest.NewRequest("GET", target, nil))
		if rec.Code != http.StatusOK {
...
//
// My first version cleared config.NetworkConfig.RPCURL in place, so an RPC that was
