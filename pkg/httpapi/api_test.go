...
func TestRejectUnknownNetwork(t *testing.T) {
	configured := []config.NetworkConfig{{ID: "gnoland1"}, {ID: "sapphire"}}
...
func TestGnoEvents(t *testing.T) {
	tx := func(hash string, ok bool, evs ...indexer.TxEvent) indexer.Transaction {
		return indexer.Transaction{Hash: hash, BlockHeight: 10, BlockTime: "2026-01-01T00:00:00Z", Success: ok,
			Response: &indexer.TxResponse{Events: evs}}
	}
	gno := func(typ string) indexer.TxEvent { return indexer.TxEvent{Typename: "GnoEvent", Type: typ} }
	storage := indexer.TxEvent{Typename: "StorageDepositEvent", Type: "StorageDeposit"}
	...
	tests := []struct {
	...
	}{
		{
			...
			txs:      []indexer.Transaction{tx("a", true, gno("Transfer"), gno("Approval"))},
			wantRows: 1,
			wantEvs:  2,
		},
		{
			...
			txs:      []indexer.Transaction{tx("a", true, storage), tx("b", true, gno("Mint"))},
			wantRows: 1,
			wantEvs:  1,
		},
		{
			name:     "filters non-gno events out of a mixed transaction",
			txs:      []indexer.Transaction{tx("a", true, storage, gno("Mint"), storage)},
			wantRows: 1,
			wantEvs:  1,
		},
		{
			name:     "a nil response is not a crash",
			txs:      []indexer.Transaction{{Hash: "a", BlockHeight: 1}},
			wantRows: 0,
		},
		{
...
func TestMergedListDoesNotDropAChain(t *testing.T) {
	...
	for i := 0; i < 40; i++ {
		rows = append(rows, indexer.Transaction{
...
func TestHandleTimeSeriesActiveRollingIsAlwaysDaily(t *testing.T) {
	...
	var pts []store.RollingActivePoint
	if err := json.Unmarshal(w.Body.Bytes(), &pts); err != nil {
...
func TestHandleCallRealmsValidatesLimit(t *testing.T) {
	api := &API{db: store.NewTestDB(t)}
	now := time.Now().UTC()
	for i := 0; i < store.RealmsWithCallsMaxLimit+20; i++ {
		store.MustCall(t, api.db, "gnoland1", fmt.Sprintf("c%d", i), i+1, now.Add(-time.Hour), "g1a", fmt.Sprintf("gno.land/r/x%d", i), "F")
	}

	w := httptest.NewRecorder()
	api.HandleCallRealms(w, httptest.NewRequest("GET", fmt.Sprintf("/api/calls/realms?limit=%d", store.RealmsWithCallsMaxLimit+20), nil))
	if w.Code != 200 {
		t.Fatalf("status = %d, want 200", w.Code)
	}
	var paths []string
	...
	if len(paths) != store.RealmsWithCallsMaxLimit {
		t.Errorf("got %d realms, want the cap of %d", len(paths), store.RealmsWithCallsMaxLimit)
	}
