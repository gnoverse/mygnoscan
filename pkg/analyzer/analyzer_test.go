...
func TestExtractImports(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			files: []indexer.MemFile{{Name: "a.gno", Body: `
				import (
					"std"
					"strings"
					"gno.land/p/demo/avl"
...
func TestExtractMsgRunImports(t *testing.T) {
	tests := []struct {
	...
	}{
		{
			...
			files: []indexer.MemFile{{Name: "main.gno", Body: `import "gno.land/p/demo/avl"`}},
			want:  []string{"gno.land/p/demo/avl"},
		},
		{
			name:  "body without a .gno name is skipped",
			files: []indexer.MemFile{{Name: "main", Body: `import "gno.land/p/demo/avl"`}},
			want:  nil,
		},
	}
...
func seedPackage(t *testing.T, db *store.DB, network, path string, files []indexer.MemFile, staleEdges []string) {
	t.Helper()
...
func TestReextractDependenciesReplacesStaleEdges(t *testing.T) {
	...
	src := []indexer.MemFile{{Name: "a.gno", Body: `package main
...
func depsOf(t *testing.T, db *store.DB, network, pkgPath string) []string {
	t.Helper()
	rows, err := db.SQL().Query(`SELECT import_path FROM dependencies WHERE network = ? AND package_path = ? ORDER BY import_path`, network, pkgPath)
	if err != nil {
...
func TestExtractImportsNeverEmitsASelfEdge(t *testing.T) {
	...
	got := NewAnalyzer(nil).ExtractImports(pkgPath, []indexer.MemFile{
